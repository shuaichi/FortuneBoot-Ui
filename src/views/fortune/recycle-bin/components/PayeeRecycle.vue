<template>
  <div>
    ...
    >
      ...
      <el-form-item v-show="isVisible(2)" label="名称：" prop="payeeName">
        <el-input
    ...
    <PureTableBar title="交易对象回收站" :columns="columns" @refresh="onSearch">
      <template v-slot="{ size, dynamicColumns }">
        ...
        >
          <template #operation="{ row }">
            <el-popconfirm
              :title="`确认恢复【${row.payeeName}】交易对象吗？`"
            ...
            <el-popconfirm
              :title="`确认删除【${row.payeeName}】交易对象吗？数据一旦删除，将不可恢复！`"
...
async function onSearch() {
  try {
    loading.value = true;
    const categoryRes = await getPayeePageApi(searchForm);
...
async function handlePutBack(row: PayeeVo) {
  await payeePutBackApi(row.bookId, row.payeeId);
  await onSearch();
}

async function handleRemove(row: PayeeVo) {
  await payeeRemoveApi(row.bookId, row.payeeId);
  await onSearch();
}
</script>
